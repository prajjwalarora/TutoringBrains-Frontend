...
const DashboardChip = (props) => {
  ...
  useEffect(() => {
    if (isChipDetailsOpen && !isReqSent) {
      sendRequest({
        token: auth.token,
        assessmentId: props.cardData.assessment.id,
      });
      setIsReqSent(true);
    }
      ...
      >
        <div className={classes["text-container"]}>
          <h3>
            ...
            {props.cardData.assessment.name || ""}
          </h3>
        </div>
        <div className={classes["timer"]}>
          <EventIcon style={{ fontSize: 20 }} />
          <span>
            {props.cardData.assessment.examDate
              ? props.cardData.assessment.examDate.split("T")[0]
              : props.cardData.assessment.classDate.split("T")[0] ||
                "2022-06-13"}
          </span>
        </div>
        <div
            ...
            <div className={classes["text-info-container"]}>
              <div className={classes["text-info"]}>
                <p>
                  ...
                  <span>{`${props.cardData.assessment.students.length}`}</span>
                </p>
              </div>
              <div className={classes["text-info"]}>
                <p>
